local M = {
  id = "CommonsBase_Std.Extract@0.1.1"
}
...
CommonsBase_Std__Extract__0_1_1 = {}
...
function rules.F_Untar(command, request)
  ...
  elseif command == "submit" then
    ...
    CommonsBase_Std__Extract__0_1_1.common_params(request, p)
    return CommonsBase_Std__Extract__0_1_1.untar(p)
  end
end
...
function rules.F_TarToZip(command, request)
  ...
  elseif command == "submit" then
    local p = {}
    CommonsBase_Std__Extract__0_1_1.common_params(request, p)
    return CommonsBase_Std__Extract__0_1_1.tartozip(p)
  end
end

function CommonsBase_Std__Extract__0_1_1.common_params(request, p)
  local tarmodver = assert(request.user.tarmodver, "please provide `tarmodver=MODULE@VERSION`")
  local tarassetpath = assert(request.user.tarassetpath, "please provide `tarassetpath=ASSETPATH`")
...
function CommonsBase_Std__Extract__0_1_1.untar(p)
  local commands = {
    -- macOS system tar
    {
...
function CommonsBase_Std__Extract__0_1_1.tartozip_win32_helper(win32_commands, p, slot)
...
function CommonsBase_Std__Extract__0_1_1.tartozip(p)
  local commands = {
    -- macOS
      -- macOS system tar
  ...
  if p.gzip or p.xz or p.bz2 then
    CommonsBase_Std__Extract__0_1_1.tartozip_win32_helper(commands, p, "Windows_x86")
    CommonsBase_Std__Extract__0_1_1.tartozip_win32_helper(commands, p, "Windows_x86_64")
    CommonsBase_Std__Extract__0_1_1.tartozip_win32_helper(commands, p, "Windows_arm64")
  else
      -- extract the .tar
        -- with [env -u] so runs on Windows slots only
        -- with [7z.exe] ...
