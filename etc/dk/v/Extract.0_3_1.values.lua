local M = {
  id = "CommonsBase_Std.Extract@0.3.1"
}

-- 0.3.1 passes `-snl-` to every Windows `7z.exe x` so a tarball that carries a
-- symbolic link whose target escapes the extraction root no longer fails the
-- extraction. 7-Zip refuses such a "dangerous link" and exits 2 even though it
-- extracted every real file; `-snl-` disables symlink restoration (unreliable on
-- Windows regardless), so the link is skipped and the command exits 0. Unix
-- extraction uses a real tar and was never affected. The menhir opam source,
-- which ships relative symlinks under demos/, hit this.
...
CommonsBase_Std__Extract__0_3_1 = {}
...
function rules.F_Untar(command, request)
  ...
  elseif command == "submit" then
    ...
    CommonsBase_Std__Extract__0_3_1.common_params(request, p)
    return CommonsBase_Std__Extract__0_3_1.untar(p)
  end
end
...
function rules.F_TarToZip(command, request)
  ...
  elseif command == "submit" then
    local p = {}
    CommonsBase_Std__Extract__0_3_1.common_params(request, p)
    return CommonsBase_Std__Extract__0_3_1.tartozip(p)
  end
end

function CommonsBase_Std__Extract__0_3_1.common_params(request, p)
  local tarmodver = assert(request.user.tarmodver, "please provide `tarmodver=MODULE@VERSION`")
  local tarassetpath = assert(request.user.tarassetpath, "please provide `tarassetpath=ASSETPATH`")
...
function CommonsBase_Std__Extract__0_3_1.append_paths(cmd, paths)
  local i = 1
...
function CommonsBase_Std__Extract__0_3_1.strip_leading(path, nstrip)
  local p = path
  local n = nstrip or 0
  local more = 1
...
function CommonsBase_Std__Extract__0_3_1.dirname(path)
  local last = 0
  local i = 1
...
function CommonsBase_Std__Extract__0_3_1.shape_extract(commands, p, slot, archive, nstrip, has_dest)
  local H = CommonsBase_Std__Extract__0_3_1
  ...
  table.insert(commands,
    H.append_paths({ p.sevenzexe_win32, "x", "-snl-", "-o" .. stage, archive }, p.paths))
  local pi = 1
...
function CommonsBase_Std__Extract__0_3_1.untar(p)
  local append_paths = CommonsBase_Std__Extract__0_3_1.append_paths
  ...
  while unix[ui] ~= nil do
    ...
    -- zst: decompress with 7zz into the working directory first (see
    -- common_params), then extract the plain tar. `-y` tolerates the repeat
    -- decompress when more than one slot builds on the same host.
    ...
    if unix[ui][1] == "toybox" then
      cmd = { p.toyboxexe, "tar", "-x" .. p.tarcompressflag .. "f", tarfile, "-C", dest }
    else
      cmd = { "/usr/bin/tar", "-x" .. p.tarcompressflag .. "f", tarfile, "-C", dest }
    end
  ...
  if p.gzip or p.xz or p.bz2 or p.zst then
    table.insert(commands, {
        ...
        p.sevenzexe_win32, "x", "-snl-", "-o.", p.tarfile, p.file_tar_basename })
    table.insert(commands, {
    ...
    while win[wi] ~= nil do
      ...
      if shaping then
        CommonsBase_Std__Extract__0_3_1.shape_extract(commands, p, slot, p.file_tar_basename, nstrip, has_dest)
      else
        table.insert(commands, append_paths({ p.sevenzexe_win32, "x", "-snl-", "-o" .. slot, p.file_tar_basename }, p.paths))
      end
      wi = wi + 1
    end
  else
    ...
    while win[wi] ~= nil do
      local slot = "${SLOT.Release." .. win[wi] .. "}"
      if shaping then
        CommonsBase_Std__Extract__0_3_1.shape_extract(commands, p, slot, p.tarfile, nstrip, has_dest)
      else
        table.insert(commands, append_paths({ p.sevenzexe_win32, "x", "-snl-", "-o" .. slot, p.tarfile }, p.paths))
      end
      wi = wi + 1
    end
  end
  ...
  while p.paths[oi] ~= nil do
    local sp = CommonsBase_Std__Extract__0_3_1.strip_leading(p.paths[oi], nstrip)
...
function CommonsBase_Std__Extract__0_3_1.tartozip_win32_helper(win32_commands, p, slot)
...
function CommonsBase_Std__Extract__0_3_1.tartozip(p)
  ...
  if p.gzip or p.xz or p.bz2 or p.zst then
    CommonsBase_Std__Extract__0_3_1.tartozip_win32_helper(commands, p, "Windows_x86")
    CommonsBase_Std__Extract__0_3_1.tartozip_win32_helper(commands, p, "Windows_x86_64")
    CommonsBase_Std__Extract__0_3_1.tartozip_win32_helper(commands, p, "Windows_arm64")
  else
      -- extract the .tar
    ...
    table.insert(commands, {
        p.coreutilsexe, "env", "-u", "${SLOT.Release.Windows_x86}", "--",
        p.sevenzexe_win32, "x", "-snl-", "-o.", p.tarfile
      })
    table.insert(commands, {
