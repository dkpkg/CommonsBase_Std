local M = {
  id = "CommonsBase_Std.Extract@0.2.1"
}
...
CommonsBase_Std__Extract__0_2_1 = {}
...
function rules.F_Untar(command, request)
  ...
  elseif command == "submit" then
    ...
    CommonsBase_Std__Extract__0_2_1.common_params(request, p)
    return CommonsBase_Std__Extract__0_2_1.untar(p)
  end
end
...
function rules.F_TarToZip(command, request)
  ...
  elseif command == "submit" then
    local p = {}
    CommonsBase_Std__Extract__0_2_1.common_params(request, p)
    return CommonsBase_Std__Extract__0_2_1.tartozip(p)
  end
end

function CommonsBase_Std__Extract__0_2_1.common_params(request, p)
  local tarmodver = assert(request.user.tarmodver, "please provide `tarmodver=MODULE@VERSION`")
  local tarassetpath = assert(request.user.tarassetpath, "please provide `tarassetpath=ASSETPATH`")
...
function CommonsBase_Std__Extract__0_2_1.append_paths(cmd, paths)
  local i = 1
...
function CommonsBase_Std__Extract__0_2_1.strip_leading(path, nstrip)
  local p = path
  local n = nstrip or 0
  local more = 1
...
function CommonsBase_Std__Extract__0_2_1.dirname(path)
  local last = 0
  local i = 1
...
function CommonsBase_Std__Extract__0_2_1.shape_extract(commands, p, slot, archive, nstrip, has_dest)
  local H = CommonsBase_Std__Extract__0_2_1
...
function CommonsBase_Std__Extract__0_2_1.untar(p)
  local append_paths = CommonsBase_Std__Extract__0_2_1.append_paths
  ...
  if p.gzip or p.xz or p.bz2 then
    ...
    while win[wi] ~= nil do
      ...
      if shaping then
        CommonsBase_Std__Extract__0_2_1.shape_extract(commands, p, slot, p.file_tar_basename, nstrip, has_dest)
      else
    ...
    while win[wi] ~= nil do
      local slot = "${SLOT.Release." .. win[wi] .. "}"
      if shaping then
        CommonsBase_Std__Extract__0_2_1.shape_extract(commands, p, slot, p.tarfile, nstrip, has_dest)
      else
  ...
  while p.paths[oi] ~= nil do
    local sp = CommonsBase_Std__Extract__0_2_1.strip_leading(p.paths[oi], nstrip)
...
function CommonsBase_Std__Extract__0_2_1.tartozip_win32_helper(win32_commands, p, slot)
...
function CommonsBase_Std__Extract__0_2_1.tartozip(p)
  local commands = {
    -- macOS
      -- macOS system tar
  ...
  if p.gzip or p.xz or p.bz2 then
    CommonsBase_Std__Extract__0_2_1.tartozip_win32_helper(commands, p, "Windows_x86")
    CommonsBase_Std__Extract__0_2_1.tartozip_win32_helper(commands, p, "Windows_x86_64")
    CommonsBase_Std__Extract__0_2_1.tartozip_win32_helper(commands, p, "Windows_arm64")
  else
      -- extract the .tar
